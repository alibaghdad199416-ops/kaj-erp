...
# Never regenerate/replace dart_defines.json here. R15 keeps the production
# Supabase/Firebase configuration byte-identical to the validated release.
Invoke-Step 'R15 Windows validation' { npm run validate:r15:windows }
...
if ($expectedPending.Count -gt 0) {
    # R14 may still be pending on a production database currently running R13.
    # Supabase applies migrations in timestamp order, so R14 always precedes R15.
    Invoke-Step "Push canonical runtime migrations to Supabase production: $($expectedPending -join ', ')" {
        npx supabase db push --linked --yes
    }
} else {
    Write-Host "R14/R15 migrations are not pending; they may already be applied. Continuing without replay." -ForegroundColor Yellow
}
...
# The R15 Edge Functions were not changed; do not redeploy them unnecessarily.
...
Write-Host "`nPASS R15 production deployment" -ForegroundColor Green
