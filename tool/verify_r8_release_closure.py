...
for rel, expected in expected_hashes.items():
    actual = sha(rel)
    assert actual == expected, f'configuration changed: {rel}'
...
for rel in ui_files:
    text = read(rel)
    if re.search(r'(?<![A-Za-z0-9_.])Text\(', text): violations.append(f'{rel}:Text')
    if re.search(r'(?<![A-Za-z0-9_.])SelectableText\(', text): violations.append(f'{rel}:SelectableText')
