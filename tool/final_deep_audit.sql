...
-- Authenticated tenant fixture for disposable CI only.
insert into auth.users(id,aud,role,email,encrypted_password,email_confirmed_at,created_at,updated_at,raw_app_meta_data,raw_user_meta_data)
select user_id,'authenticated','authenticated',email,crypt('Audit-R56-R68-Password!',gen_salt('bf')),now(),now(),now(),'{}'::jsonb,'{"audit":true}'::jsonb
...
insert into public.profiles(id,full_name,is_active) select user_id,'Final Deep Audit',true from audit_constants
on conflict(id) do update set full_name=excluded.full_name,is_active=true;
insert into public.company_memberships(company_id,user_id,role_code,is_system_admin,is_active) select company_id,user_id,'owner',true,true from audit_constants
...
-- Security contract: tenant tables must have RLS and client roles must not have direct writes.
do $$
declare v text;
begin
  ...
  where table_schema='public' and grantee in('anon','authenticated') and privilege_type in('INSERT','UPDATE','DELETE')
    and table_name not like 'supabase_%' and table_name<>'schema_migrations';
  if v is not null then raise exception 'FINAL_DEEP_AUDIT DIRECT_DML_GRANTS: %',v; end if;
end $$;

-- Cross-tenant command must be rejected.
do $$
...
begin
  ...
  exception when others then
    if sqlstate<>'42501' and sqlerrm not like '%tenant_denied%' and sqlerrm not like '%company_membership_required%' then raise; end if;
  end;
end $$;

-- Business fixtures.
insert into public.erp_customers(company_id,id,data,created_by,updated_by) select company_id,'audit-customer',jsonb_build_object('name','Final Audit Customer','currency','USD','isActive',true),user_id,user_id from audit_constants
on conflict(company_id,id) do update set data=excluded.data,is_deleted=false,deleted_at=null,updated_at=now();
insert into public.erp_suppliers(company_id,id,data,created_by,updated_by) select company_id,'audit-supplier',jsonb_build_object('name','Final Audit Supplier','currency','USD','isActive',true),user_id,user_id from audit_constants
on conflict(company_id,id) do update set data=excluded.data,is_deleted=false,deleted_at=null,updated_at=now();
insert into public.erp_warehouses(company_id,id,data,created_by,updated_by) select company_id,'audit-warehouse',jsonb_build_object('code','AUDIT','name','Final Audit Warehouse','isActive',true),user_id,user_id from audit_constants
on conflict(company_id,id) do update set data=excluded.data,is_deleted=false,deleted_at=null,updated_at=now();
insert into public.erp_cars(company_id,id,data,created_by,updated_by) select company_id,'audit-car',jsonb_build_object('brand','Audit','model','E2E','chassis','AUDIT-R56-R68','status','available','warehouse_id','audit-warehouse','warehouseId','audit-warehouse','currency','USD'),user_id,user_id from audit_constants
on conflict(company_id,id) do update set data=excluded.data,is_deleted=false,deleted_at=null,updated_at=now();

-- Sales: success, idempotent retry, invalid-master rollback.
do $$
...
-- Purchases: success and idempotent retry.
do $$
...
-- Optimistic concurrency: a stale snapshot must be rejected.
do $$
declare initial_updated timestamptz; first_updated timestamptz; payload jsonb; rejected boolean:=false;
begin
  insert into public.erp_records(company_id,entity_type,record_id,payload,updated_at)
  values('quality-line','opportunities','audit-concurrency',jsonb_build_object('id','audit-concurrency','status','pending','auditMarker','AUDIT-R56-R68'),now())
  on conflict(company_id,entity_type,record_id) do update set payload=excluded.payload,deleted_at=null,updated_at=now();
  select updated_at into initial_updated from public.erp_records where company_id='quality-line' and entity_type='opportunities' and record_id='audit-concurrency';
  ...
  select updated_at into first_updated from public.erp_records where company_id='quality-line' and entity_type='opportunities' and record_id='audit-concurrency';
  ...
  begin
    perform public.erp_r49_opportunity_command('save',payload);
  exception when others then rejected:=sqlstate='40001' or sqlerrm like '%stale_record_conflict%'; end;
  if not rejected then raise exception 'stale update accepted'; end if;
end $$;

-- Document storage: foreign tenant path rejected; canonical tenant path accepted.
do $$
...
begin
  ...
  begin
    perform public.erp_register_cloud_document_blob(c,d,v,'other-company/file.bin',4);
    raise exception 'foreign document path accepted';
  exception when others then if sqlerrm not like '%invalid_document_storage_path%' then raise; end if; end;
