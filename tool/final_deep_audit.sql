...
begin
  ...
  begin
    perform public.erp_v2300_create_sales_order(other_company,jsonb_build_object('customerId','audit-customer','currency','USD','exchangeRate',1,'items',jsonb_build_array(jsonb_build_object('itemType','car','itemId','audit-car','quantity',1,'unitPrice',10,'description','Audit cross-tenant car'))));
...
begin
  ...
  select public.erp_v2300_create_sales_order(c,jsonb_build_object('customerId','audit-customer','currency','USD','exchangeRate',1,'opportunityId','AUDIT-SALES-OPP','items',jsonb_build_array(jsonb_build_object('itemType','car','itemId','audit-car','quantity',1,'unitPrice',1000,'description','Final Audit Vehicle Sale')))) into a;
  if a is null then raise exception 'sales create returned null'; end if;
  select public.erp_v2300_create_sales_order(c,jsonb_build_object('customerId','audit-customer','currency','USD','exchangeRate',1,'opportunityId','AUDIT-SALES-OPP','items',jsonb_build_array(jsonb_build_object('itemType','car','itemId','audit-car','quantity',1,'unitPrice',1000,'description','Final Audit Vehicle Sale')))) into b;
  ...
  begin
    perform public.erp_v2300_create_sales_order(c,jsonb_build_object('customerId','missing','currency','USD','exchangeRate',1,'items',jsonb_build_array(jsonb_build_object('itemType','car','itemId','audit-car','quantity',1,'unitPrice',10,'description','Invalid customer rollback test'))));
...
begin
  select company_id into c from audit_constants;
  select public.erp_v2300_create_purchase_order(c,jsonb_build_object('supplierId','audit-supplier','currency','USD','exchangeRate',1,'opportunityId','AUDIT-PURCHASE-OPP','items',jsonb_build_array(jsonb_build_object('itemType','car','itemId','audit-car','quantity',1,'unitCost',700,'description','Final Audit Vehicle Purchase')))) into a;
  if a is null then raise exception 'purchase create returned null'; end if;
  select public.erp_v2300_create_purchase_order(c,jsonb_build_object('supplierId','audit-supplier','currency','USD','exchangeRate',1,'opportunityId','AUDIT-PURCHASE-OPP','items',jsonb_build_array(jsonb_build_object('itemType','car','itemId','audit-car','quantity',1,'unitCost',700,'description','Final Audit Vehicle Purchase')))) into b;
