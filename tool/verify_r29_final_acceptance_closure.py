...
need('cash voucher bundled logo available on web', 'rootBundle.load' in text(pdf_paths[0]) and 'if (!kIsWeb)' not in text(pdf_paths[0]))
...
need('web pdf blob download path', 'browser_download.saveBinary' in pdf_web and 'html.Blob' in binary_web and 'html.AnchorElement' in binary_web and '..download = fileName' in binary_web and 'html.window.open(' not in pdf_web)
need('excel blob lifetime safe', 'Future<void>.delayed' in text('lib/core/exporting/excel_download_service_web.dart'))
need('binary blob lifetime safe', 'triggerBrowserDownload' in binary_web and 'Future<void>.delayed' in download_lifecycle)
