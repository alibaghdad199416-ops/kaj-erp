...
# IMPORTANT: Deployment runs from an installed workspace. It MUST NOT execute
# verify:delivery after npm/flutter/build have generated local artifacts.
# It rebuilds and validates the current source instead, so Firebase always
# receives build/web produced by the same source that passed analyzer/tests.
Invoke-Step 'R19 installed-workspace validation and fresh web build' {
    npm run validate:r19:workspace
}
...
Invoke-Step 'Deploy freshly validated build/web to Firebase Hosting' {
    npx firebase-tools deploy --only hosting --project $FirebaseProject --non-interactive
}

Write-Host "`nPASS R19 production deployment" -ForegroundColor Green
