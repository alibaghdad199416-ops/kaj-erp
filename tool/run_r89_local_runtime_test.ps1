...
$TestFile = Join-Path $ProjectRoot 'supabase\tests\verify_r89_phase11_runtime.sql'

if (-not (Test-Path -LiteralPath $TestFile -PathType Leaf)) {
  throw "R89 runtime SQL is missing: $TestFile"
}
...
$remote = '/tmp/verify_r89_phase11_runtime.sql'
Write-Host 'Running R89 Phase 11 runtime test against LOCAL Supabase only...' -ForegroundColor Cyan
& docker.exe cp $TestFile "${Container}:$remote"
if ($LASTEXITCODE -ne 0) { throw 'Could not copy R89 runtime SQL into the LOCAL database container.' }

& docker.exe exec $Container psql -U postgres -d postgres -X -v ON_ERROR_STOP=1 -f $remote
if ($LASTEXITCODE -ne 0) { throw 'R89 LOCAL PostgreSQL runtime test failed.' }

Write-Host 'PASS: R89 Phase 11 LOCAL PostgreSQL runtime test.' -ForegroundColor Green
