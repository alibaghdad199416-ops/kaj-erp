...
class SalesWorkflowPage extends StatefulWidget {
  const SalesWorkflowPage({super.key, this.initialOrderId});
...
class _SalesWorkflowPageState extends State<SalesWorkflowPage> {
  ...
  @override
  void didUpdateWidget(covariant SalesWorkflowPage oldWidget) {
  ...
  Future<void> _delivery(String id) async {
    if (!await _requireWorkflowAction('delivery.create')) return;
  ...
  Future<void> _invoice(String id) async => _runWorkflowAction(
    'invoice.create',
    () => _repository.createInvoiceDraft(id),
    orderId: id,
  );
  ...
  Future<void> _editOrder(String orderId) async {
    if (!await PermissionAction.require(context, 'sales.update')) return;
  ...
  List<String> _legacyPermissionsForAction(String action) => switch (action) {
    'order.approve' => const <String>['sales.approve'],
    'delivery.create' => const <String>['sales.update'],
    'delivery.approve' => const <String>[
      'sales.approve',
      'sales.update',
      'sales.create',
    ],
    ...
    'invoice.approve' => const <String>['sales.approve'],
    'payment' => const <String>['cashbox.receipt'],
    'reverse' => const <String>['sales.cancel', 'sales.update', 'sales.delete'],
    _ => const <String>['sales.view'],
  };
  ...
  bool _allowsGranularAction(String action) {
    ...
    if (!access.hasRestrictedActions('sales')) return true;
    return access.hasPermission('sales.$action');
  }

  Future<bool> _requireWorkflowAction(String action) async {
    if (_allowsGranularAction(action)) return true;
    final access = context.read<AccessController>();
    await access.recordDeniedAccess('sales.$action');
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: AppText(
            context.l10n.isArabic
                ? 'ليس لديك صلاحية لتنفيذ هذه المرحلة من دورة البيع.'
                : 'You do not have permission to perform this sales workflow stage.',
          ),
        ),
      );
    }
    ...
    final actions = <CommercialWorkflowAction>[
      if ((status == 'draft' || status == 'cancelled') &&
        ...
        CommercialWorkflowAction(
          label: _bi('تصديق أمر البيع', 'Approve sales order'),
      ...
      if (_serverFlag(
          ...
          _allowsGranularAction('delivery.create'))
        CommercialWorkflowAction(
          label: _bi('إنشاء إذن تجهيز مخزني', 'Create warehouse delivery'),
          icon: Icons.local_shipping_outlined,
          primary: true,
          onPressed: () => _delivery(order['id'].toString()),
        ),
      if (_serverFlag(
            order,
            'canApproveDelivery',
          ...
          _allowsGranularAction('delivery.approve'))
        CommercialWorkflowAction(
          label: _bi('تصديق إذن التجهيز', 'Approve warehouse delivery'),
      ...
      if (_serverFlag(
        ...
        CommercialWorkflowAction(
          label: _bi('حذف أو عكس إذن التجهيز', 'Reverse warehouse delivery'),
      ...
      if (_serverFlag(
        ...
        CommercialWorkflowAction(
          label: _bi('إنشاء فاتورة بيع', 'Create sales invoice'),
      ...
      if (_serverFlag(
        ...
        CommercialWorkflowAction(
          label: _bi('تصديق فاتورة البيع', 'Approve sales invoice'),
      ...
      if (_serverFlag(
          ...
          _allowsGranularAction('reverse'))
        CommercialWorkflowAction(
          label: _bi('حذف أو عكس فاتورة البيع', 'Reverse sales invoice'),
          icon: Icons.settings_backup_restore_rounded,
          onPressed: () => _cancelInvoice(order),
        ),
      if (context.read<AccessController>().hasPermission('sales.update'))
        CommercialWorkflowAction(
      ...
      if ((status == 'draft' || status == 'cancelled') &&
          context.read<AccessController>().hasPermission('sales.delete'))
        CommercialWorkflowAction(
      ...
      if (status != 'draft' &&
          status != 'cancelled' &&
          context.read<AccessController>().hasPermission('sales.cancel'))
        CommercialWorkflowAction(
      ...
      : Column(
          children: [
            CommercialWorkflowFilterBar(
              ...
              onCreate:
                  context.read<AccessController>().hasPermission('sales.create')
                  ? _newDraft
                  : null,
                      ...
                      child: Padding(
                        ...
                        child: KajCommercialEmptyState(
                          ...
                          actionLabel:
                              ...
                              ? AppTranslation.translate('مسودة بيع جديدة')
                              : null,
                          onAction:
                              context.read<AccessController>().hasPermission(
                                'sales.create',
