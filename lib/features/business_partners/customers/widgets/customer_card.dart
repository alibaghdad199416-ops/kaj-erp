...
class CustomerCard extends StatelessWidget {
  const CustomerCard({super.key, required this.customer, this.onView, this.onEdit, this.onDelete});
  ...
  Widget _visible(String field, Widget child) => FieldPermissionVisibility(
    resource: 'customers', field: field, viewPermission: 'customers.view', child: child,
  );
      ...
      child: Padding(
        ...
        child: LayoutBuilder(builder: (context, constraints) {
          ...
          final avatar = _visible('photo', ClipOval(child: SizedBox(
            ...
            child: _photoBytes == null
                ? ColoredBox(
                    ...
                    child: Center(child: AppText(_initials, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w900))),
                  )
          ...
          final info = Wrap(spacing: 5, runSpacing: 4, children: [
            ...
            _visible('address', PartnerCompactValue(t('العنوان', 'Address'), customer.address)),
            _visible('createdAt', PartnerCompactValue(t('تاريخ الإنشاء', 'Created at'), customer.createdAt)),
          ]);
          final actions = Wrap(spacing: 2, runSpacing: 2, children: [
            ...
            if (onEdit != null) PartnerCompactAction(Icons.edit_outlined, t('تعديل', 'Edit'), onEdit!),
            if (onDelete != null) PartnerCompactAction(Icons.delete_outline, t('حذف', 'Delete'), onDelete!, destructive: true),
          ]);
          if (compact) {
  ...
  String get _initials {
    final parts = customer.name.trim().split(RegExp(r'\s+')).where((e) => e.isNotEmpty).toList();
