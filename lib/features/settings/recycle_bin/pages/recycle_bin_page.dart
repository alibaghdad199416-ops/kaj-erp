...
class _RecycleBinPageState extends State<RecycleBinPage> {
    ...
    switch (field) {
      case 'deletedAt':
        return (left.deletedAt ?? DateTime.fromMillisecondsSinceEpoch(0))
            .compareTo(
              right.deletedAt ?? DateTime.fromMillisecondsSinceEpoch(0),
            );
      case 'title':
    ...
    try {
      ...
      _message(
        userFacingError(
          ...
          englishFallback: 'Unable to restore the record. Please try again.',
        ),
        error: true,
      );
    }
    ...
    try {
      ...
      _message(
        userFacingError(
          ...
          englishFallback:
              'Unable to permanently delete the batch. Please try again.',
        ),
        error: true,
      );
    }
  ...
  ExportDocument _report() {
    ...
    return ExportDocument(
      ...
      metadata: {
        ...
        'Type filter': _selectedType == null
            ? 'All'
            : _typeLabel(_selectedType!, isArabic: exportArabic),
    ...
    try {
      ...
      _message(
        userFacingError(
          error,
          isArabic: context.l10n.isArabic,
          arabicFallback: 'تعذر إنشاء تقرير PDF.',
          englishFallback: 'Unable to create the PDF report.',
        ),
        error: true,
      );
    } finally {
    ...
    try {
      ...
      _message(
        userFacingError(
          error,
          isArabic: context.l10n.isArabic,
          arabicFallback: 'تعذر إنشاء تقرير Excel.',
          englishFallback: 'Unable to create the Excel report.',
        ),
        error: true,
      );
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final filterOptions = types
        .map(
          (type) => UnifiedQueryFilterOption(
            token: UnifiedFilterToken(
              key: 'entityType',
              label: context.l10n.isArabic ? 'نوع السجل' : 'Record type',
              value: type,
              valueLabel: _typeLabel(type),
            ),
    ...
    final sortOptions = <UnifiedQuerySortOption>[
      UnifiedQuerySortOption(
        rule: const UnifiedSortRule(field: 'deletedAt', label: 'التاريخ'),
        icon: Icons.schedule_outlined,
      ),
      UnifiedQuerySortOption(
        rule: const UnifiedSortRule(field: 'title', label: 'العنوان'),
        icon: Icons.title_outlined,
      ),
      UnifiedQuerySortOption(
        rule: const UnifiedSortRule(field: 'type', label: 'النوع'),
        icon: Icons.category_outlined,
      ),
      UnifiedQuerySortOption(
        rule: const UnifiedSortRule(field: 'related', label: 'الارتباطات'),
        ...
        children: [
          KajAdminWorkspace(
            ...
            metrics: <KajAdminMetricData>[
              ...
              KajAdminMetricData(
                label: context.l10n.isArabic ? 'النوع' : 'Type',
            ...
            children: [
              if (canExport)
                OutlinedButton.icon(
                  ...
                  label: AppText(context.l10n.isArabic ? 'PDF' : 'PDF'),
                ),
              if (canExport)
                OutlinedButton.icon(
                  onPressed: _items.isEmpty || _exporting ? null : _exportExcel,
                  icon: const Icon(Icons.table_view_outlined),
                  label: AppText(context.l10n.isArabic ? 'Excel' : 'Excel'),
                ),
            ],
          ),
          const SizedBox(height: 10),
          if (_loading)
            KajAdminState(
              ...
              message: context.l10n.isArabic
                  ? 'يتم جلب السجلات المحذوفة.'
                  : 'Fetching deleted records.',
            )
          else if (_error != null)
            KajAdminState(
              kind: KajAdminStateKind.error,
              title: context.l10n.isArabic
                  ? 'تعذر تحميل السجلات'
                  : 'Unable to load records',
              message: _error!,
              actionLabel: context.l10n.isArabic ? 'إعادة المحاولة' : 'Retry',
              onAction: _load,
            )
          else if (_items.isEmpty)
            KajAdminState(
              kind: KajAdminStateKind.empty,
              title: context.l10n.isArabic ? 'لا توجد نتائج' : 'No results',
              message: context.l10n.isArabic
                  ? 'لا توجد سجلات مطابقة للبحث والفلاتر الحالية.'
                  : 'No deleted records match the current query.',
            )
          else
            ..._items.map(
              (item) => Card(
                ...
                child: ListTile(
                  ...
                  trailing: Wrap(
                    ...
                    children: [
                      IconButton(
                        tooltip: context.l10n.isArabic ? 'تفاصيل' : 'Details',
