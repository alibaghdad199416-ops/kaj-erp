...
class _NotificationCenterPageState extends State<NotificationCenterPage> {
  final NotificationCenterRepository _repository = NotificationCenterRepository();
  ...
  void initState() {
    ...
    _subscription = AppDataChangeBus.instance.events.listen((event) {
      const relevantSources = <String>{
        'notifications', 'sales', 'purchases', 'maintenance', 'inventory',
        'cars', 'customers', 'suppliers', 'opportunities', 'accounting',
        'cashbox', 'expenses',
      };
      if (!relevantSources.contains(event.source)) return;
      _refreshDebounce?.cancel();
      _refreshDebounce = Timer(const Duration(milliseconds: 700), () => unawaited(_load()));
    });
    unawaited(_load());
  }
  ...
  Future<void> _openPersistentNotification(Map<String, Object?> notification) async {
    final id = '${notification['id'] ?? ''}'.trim();
    if (id.isNotEmpty && !_asBool(notification['isRead'])) {
      try {
        await _repository.markAsRead(id);
      } catch (error, stackTrace) {
        AppLogger.debug('Notification read-state sync failed: $error\n$stackTrace');
      }
    }
    if (!mounted) return;
    AppModuleNavigation.open(context, _routeForReference('${notification['referenceType'] ?? ''}'));
  }
  ...
  List<NotificationAlert> _visibleAlerts() {
    ...
    return UnifiedQueryExecutor<NotificationAlert>(
      ...
      sort: (left, right, field) {
        if (field == 'title') return UnifiedFilterEngine.normalize(left.title).compareTo(UnifiedFilterEngine.normalize(right.title));
        if (field == 'amount') return (left.amount ?? 0).compareTo(right.amount ?? 0);
        return 0;
      },
  ...
  List<Map<String, Object?>> _visiblePersistent() {
    return UnifiedQueryExecutor<Map<String, Object?>>(
      criteriaBuilder: (state) => UnifiedFilterCriteria(
        ...
        statuses: {
          for (final token in state.filters.where((e) => e.key == _severityKey)) '${token.value}',
        },
      ),
      filterAdapter: UnifiedFilterAdapter<Map<String, Object?>>(
        searchableText: (item) => <Object?>[
          item['titleAr'], item['titleEn'], item['bodyAr'], item['bodyEn'],
          item['type'], item['referenceType'], item['userName'], item['user_name'],
        ],
        status: (item) => '${item['type'] ?? 'info'}',
      ),
      sort: (left, right, field) {
        if (field == 'title') {
          final l = UnifiedFilterEngine.normalize(left['titleAr'] ?? left['titleEn']);
          final r = UnifiedFilterEngine.normalize(right['titleAr'] ?? right['titleEn']);
          return l.compareTo(r);
        }
        if (field == 'createdAt') {
          final l = DateTime.tryParse('${left['createdAt'] ?? left['created_at'] ?? ''}');
          final r = DateTime.tryParse('${right['createdAt'] ?? right['created_at'] ?? ''}');
          return (l ?? DateTime.fromMillisecondsSinceEpoch(0)).compareTo(r ?? DateTime.fromMillisecondsSinceEpoch(0));
        }
        return 0;
      },
  ...
  Widget build(BuildContext context) {
    ...
    final filterOptions = <UnifiedQueryFilterOption>[
      ...
      UnifiedQueryFilterOption(token: UnifiedFilterToken(key: _severityKey, label: ar ? 'النوع' : 'Type', value: 'warning', valueLabel: ar ? 'تحذير' : 'Warning'), icon: Icons.warning_amber_rounded),
      UnifiedQueryFilterOption(token: UnifiedFilterToken(key: _severityKey, label: ar ? 'النوع' : 'Type', value: 'info', valueLabel: ar ? 'معلومة' : 'Information'), icon: Icons.info_outline_rounded),
    ];
    final sortOptions = <UnifiedQuerySortOption>[
      UnifiedQuerySortOption(rule: UnifiedSortRule(field: 'createdAt', label: ar ? 'التاريخ' : 'Date', descending: true), icon: Icons.schedule_rounded),
      UnifiedQuerySortOption(rule: UnifiedSortRule(field: 'title', label: ar ? 'العنوان' : 'Title'), icon: Icons.title_rounded),
    ];
        ...
        child: ListView(
          ...
          children: [
            KajSignaturePageHero(
              ...
              subtitle: ar ? 'مركز موحد للتنبيهات المهمة والإشعارات المحفوظة والإجراءات التي تحتاج إلى اهتمامك.' : 'A unified center for critical alerts, saved notifications and actions that require your attention.',
              icon: Icons.notifications_active_outlined,
              metrics: <KajSignatureMetricData>[
                KajSignatureMetricData(label: ar ? 'غير مقروء' : 'UNREAD', value: '$_unreadCount', icon: Icons.mark_email_unread_outlined),
                KajSignatureMetricData(label: ar ? 'حرج' : 'CRITICAL', value: '$critical', icon: Icons.crisis_alert_rounded, accent: KajDesignTokens.danger),
            ...
            if (_loading)
              ...
              _StateCard(icon: Icons.error_outline, title: ar ? 'حدث خطأ' : 'Something went wrong', message: _error!, actionLabel: ar ? 'إعادة المحاولة' : 'Try again', onAction: _load)
            else ...[
              if (visiblePersistent.isNotEmpty) ...[
                _SectionHeader(title: ar ? 'الإشعارات المحفوظة' : 'Saved notifications', subtitle: ar ? 'غير المقروء: $_unreadCount' : 'Unread: $_unreadCount'),
                const SizedBox(height: 8),
                ...visiblePersistent.map((notification) => _PersistentNotificationCard(
                  notification: notification,
                  onOpen: () => unawaited(_openPersistentNotification(notification)),
                  onMarkRead: () => unawaited(_markAsRead('${notification['id'] ?? ''}')),
                  onArchive: () => unawaited(_archive('${notification['id'] ?? ''}')),
                )),
                const SizedBox(height: 12),
              ],
              _SectionHeader(title: ar ? 'التنبيهات التشغيلية' : 'Operational alerts', subtitle: ar ? 'تُحدّث مباشرة من بيانات النظام الحالية' : 'Updated directly from live system data'),
              const SizedBox(height: 8),
              if (visibleAlerts.isEmpty && visiblePersistent.isEmpty)
                _StateCard(icon: Icons.notifications_none_rounded, title: ar ? 'لا توجد تنبيهات' : 'No notifications', message: ar ? 'لا توجد نتائج مطابقة للبحث أو الفلاتر الحالية.' : 'No notifications match the current search or filters.')
              else if (visibleAlerts.isEmpty)
                _StateCard(icon: Icons.notifications_none_rounded, title: ar ? 'لا توجد تنبيهات تشغيلية' : 'No operational alerts', message: ar ? 'لا توجد نتائج مطابقة للتصفية الحالية.' : 'No operational alerts match the current filter.')
              else
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      AppText(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900)),
      const SizedBox(height: 2),
      AppText(subtitle, style: TextStyle(color: Theme.of(context).colorScheme.outline, fontSize: 12)),
    ],
  );
}

class _PersistentNotificationCard extends StatelessWidget {
  const _PersistentNotificationCard({required this.notification, required this.onOpen, required this.onMarkRead, required this.onArchive});
  ...
  Widget build(BuildContext context) {
    ...
    final title = _localizedValue(notification, isArabic ? 'titleAr' : 'titleEn', isArabic ? 'titleEn' : 'titleAr', fallback: 'إشعار');
    final body = _localizedValue(notification, isArabic ? 'bodyAr' : 'bodyEn', isArabic ? 'bodyEn' : 'bodyAr');
...
class _MetaText extends StatelessWidget {
  ...
  final String text;
  @override
  Widget build(BuildContext context) => AppText(text, style: TextStyle(fontSize: 10.5, color: Theme.of(context).colorScheme.outline));
}
...
class _AlertCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final tone = switch (alert.severity) { NotificationSeverity.critical => scheme.error, NotificationSeverity.warning => KajDesignTokens.warning, NotificationSeverity.info => scheme.primary };
    final ar = context.l10n.isArabic;
...
class _StateCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(child: Padding(
    ...
    child: Column(children: [
      ...
      const SizedBox(height: 10),
      AppText(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
      const SizedBox(height: 5),
      AppText(message, textAlign: TextAlign.center, style: const TextStyle(fontSize: 12)),
      if (onAction != null && actionLabel != null) ...[const SizedBox(height: 12), FilledButton(onPressed: onAction, child: AppText(actionLabel!))],
    ]),
  ));
}

String _localizedValue(Map<String, Object?> values, String primary, String secondary, {String fallback = ''}) {
...
bool _asBool(Object? value) => value is bool ? value : '${value ?? ''}'.toLowerCase() == 'true';

String _routeForReference(String referenceType) {
  final value = referenceType.trim().toLowerCase();
  if (value.contains('sale') || value.contains('delivery') || value.contains('customer_invoice')) return AppRouteNames.sales;
  if (value.contains('purchase') || value.contains('receipt') || value.contains('supplier_invoice')) return AppRouteNames.purchases;
  if (value.contains('maintenance')) return AppRouteNames.maintenance;
  if (value.contains('service') || value.contains('opportunity')) return AppRouteNames.customerService;
  if (value.contains('customer') || value.contains('supplier')) return AppRouteNames.businessPartners;
  if (value.contains('product')) return AppRouteNames.products;
  if (value.contains('stock') || value.contains('warehouse') || value.contains('car')) return AppRouteNames.inventory;
  if (value.contains('payment') || value.contains('journal') || value.contains('account') || value.contains('installment')) return AppRouteNames.accounting;
