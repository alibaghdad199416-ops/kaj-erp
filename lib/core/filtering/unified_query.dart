...
class UnifiedQueryState {
  ...
  UnifiedQueryState removeSortAt(int index) { if (index < 0 || index >= sorts.length) return this; final next = sorts.toList(growable: true)..removeAt(index); return copyWith(sorts: next); }
...
class UnifiedQuery<T> {
  ...
  UnifiedQuery<T> removeSortAt(int index) { if (index < 0 || index >= sorts.length) return this; final next = sorts.toList(growable: true)..removeAt(index); return copyWith(sorts: List<UnifiedSortCriterion<T>>.unmodifiable(next)); }
...
class UnifiedQueryController extends ChangeNotifier {
  ...
  void setSearch(String value) { if (_state.search == value) return; _state = _state.copyWith(search: value); notifyListeners(); }
  void setFilters(Iterable<UnifiedFilterToken> values) { final next = List<UnifiedFilterToken>.unmodifiable(values); if (listEquals(_state.filters, next)) return; _state = _state.copyWith(filters: next); notifyListeners(); }
  void addFilter(UnifiedFilterToken token) { final next = _state.filters.where((item) => item.key != token.key).toList(growable: false); setFilters([...next, token]); }
  void removeFilter(UnifiedFilterToken token) { final next = _state.removeFilter(token); if (next == _state) return; _state = next; notifyListeners(); }
  void removeFilterKey(String key) { final next = _state.removeFilterKey(key); if (next == _state) return; _state = next; notifyListeners(); }
  void setSorts(Iterable<UnifiedSortRule> values) { final next = List<UnifiedSortRule>.unmodifiable(values); if (listEquals(_state.sorts, next)) return; _state = _state.copyWith(sorts: next); notifyListeners(); }
  void addSort(UnifiedSortRule rule) { final next = List<UnifiedSortRule>.from(_state.sorts); final index = next.indexWhere((item) => item.field == rule.field); if (index >= 0) { final existing = next[index]; next[index] = existing.copyWith(descending: !existing.descending); } else { next.add(rule); } setSorts(next); }
  void removeSort(String field) { final next = _state.removeSort(field); if (next == _state) return; _state = next; notifyListeners(); }
  void removeSortAt(int index) { final next = _state.removeSortAt(index); if (next == _state) return; _state = next; notifyListeners(); }
  void clear() { if (_state.isEmpty) return; _state = const UnifiedQueryState(); notifyListeners(); }
}
