...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    new.company_id::text||':maintenance-invoice:'||new.parent_id::text,0));
  if exists (
    select 1 from public.erp_commercial_workflow_documents d
    where d.company_id=new.company_id and d.module='maintenance'
      and d.document_type='invoice' and d.parent_id=new.parent_id
      and d.id is distinct from new.id and not d.is_deleted
...
on public.erp_commercial_workflow_documents for each row
execute function public.erp_r49_guard_single_active_maintenance_invoice();

create or replace function public.erp_r49_validate_maintenance_invoice_state(
...
begin
  ...
  where company_id=p_company_id and id=p_order_id and not is_deleted for share;
  if not found then raise exception 'maintenance_order_not_found'; end if;
  if o.pricing_type<>'paid' or coalesce(o.sale_price,0)<=0 then
    raise exception 'paid_maintenance_invoice_required';
  end if;
  if upper(coalesce(o.currency_code,'')) not in ('IQD','USD') then
    raise exception 'maintenance_currency_invalid';
  end if;
  if o.customer_id is null then raise exception 'paid_maintenance_customer_required'; end if;
  if o.workflow_stage not in ('invoice_draft','invoice_approved','paid','completed') then
    raise exception 'maintenance_invoice_stage_invalid';
  end if;
end;
$$;
