-- Forward-only migration-order repair.
-- 20260803010000_phase2_fifo_recycle_timeline.sql performs writes against
-- legacy erp_records rows whose company_id is a tenant slug (for example
-- "quality-line"). The enterprise audit trigger installed by
-- 20260729001000 originally casts that value directly to uuid. The later
-- compatibility migration fixed the trigger, but was ordered after the
-- writes, so a fresh database reset could emit audit warnings during 030100.
-- Install the compatibility boundary before those writes execute.
...
begin
  ...
  if tg_op = 'DELETE' then return old; else return new; end if;
exception when others then
  -- Audit failures must remain non-blocking, but malformed tenant identifiers
  -- must never reach a uuid cast and generate noisy migration warnings.
