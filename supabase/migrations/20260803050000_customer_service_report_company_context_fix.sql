-- Correct the customer-service/opportunity report company context.
-- The UUID overload of erp_active_company_context authorizes membership and
-- returns only the UUID. The legacy erp_records rows remain keyed by company slug,
-- so the slug must be read from public.companies after authorization.
