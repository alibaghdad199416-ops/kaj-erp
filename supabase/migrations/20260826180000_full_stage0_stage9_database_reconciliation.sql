...
-- Cross-stage closure: keep canonical reconciliation counters cumulative
-- across every optional master relation. The previous implementation
-- overwrote scanned/reconciled with the current table's result, which made
-- the returned health contract dependent on the last existing relation.
...
begin
  ...
  foreach v_table in array array[
    ...
    if to_regclass('public.' || v_table) is not null then
      execute format(
        'select count(*) from public.%I r where r.company_id=$1',
        v_table
      ...
      execute format(
        ...
        || 'deleted_at=coalesce(r.deleted_at,now()),updated_at=now(),'
        || 'version=coalesce(r.version,0)+1 '
        || 'where r.company_id=$1 and not coalesce(r.is_deleted,false) '
        || 'and public.erp_r15_pending_delete_exists($1,$2,r.id)',
        v_table
      ) using p_company_id, v_table;
      get diagnostics v_table_reconciled = row_count;
      v_touched := v_touched + coalesce(v_table_reconciled, 0);
    end if;
  end loop;
