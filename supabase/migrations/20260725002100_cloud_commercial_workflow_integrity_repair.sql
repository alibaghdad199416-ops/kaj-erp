-- Repair migration for databases that already applied phases 23 and 25.
-- It aligns commercial master-data references with the normalized text IDs,
-- removes obsolete UUID overloads, and reapplies the corrected cloud RPCs.

begin;

-- Obsolete overloads created before normalized master-data IDs became text.
drop function if exists public.erp_add_cloud_partner_activity(uuid,uuid,text,text,text,timestamptz);
drop function if exists public.erp_open_cloud_service_case(uuid,uuid,text,text,text);
drop function if exists public.erp_create_cloud_sales_order(uuid,uuid,text,numeric,jsonb,uuid,numeric,text);
drop function if exists public.erp_create_cloud_purchase_order(uuid,uuid,text,numeric,jsonb,numeric,text);
drop function if exists public.erp_update_cloud_sales_order(uuid,uuid,uuid,text,numeric,numeric,jsonb,text);
drop function if exists public.erp_update_cloud_purchase_order(uuid,uuid,uuid,text,numeric,numeric,jsonb,text);
drop function if exists public.erp_create_cloud_sales_delivery(uuid,uuid,uuid,text);
drop function if exists public.erp_create_cloud_purchase_receipt(uuid,uuid,uuid,text);
...
begin
  for item in
    select * from (values
      ('erp_reservations','car_id'),
      ('erp_reservations','customer_id'),
      ('erp_partner_activities','partner_id'),
      ('erp_service_cases','partner_id'),
      ('erp_sales_orders_cloud','customer_id'),
      ('erp_sales_orders_cloud','opportunity_id'),
      ('erp_sales_order_items_cloud','item_id'),
      ('erp_purchase_orders_cloud','supplier_id'),
      ('erp_purchase_order_items_cloud','item_id'),
      ('erp_commercial_workflow_documents','warehouse_id')
...
-- Reapply corrected phase-23 functions.
...
-- Reapply corrected phase-25 read models.
