...
-- Their volatility contract must therefore be VOLATILE.
do $$
declare
  r record;
  v_names text[] := array[
    ...
    'erp_r9_cloud_dashboard_snapshot','erp_r9_cloud_cash_currency_summary',
...
begin
  for r in
    ...
    where n.nspname='public' and p.proname=any(v_names) and p.provolatile <> 'v'
  loop
