-- Quality Line ERP 18.9.24 / V7.5.4
-- Repairs HTTP 409 conflicts in linked multi-currency cashbox transfers.
-- The conflict was caused by a unique chart-of-accounts code already owned by
-- a historical FX clearing account with a different account_id.
begin;

create or replace function public.erp_ensure_fx_clearing_account(
  p_company_id uuid,
  p_currency text
) returns text
...
begin
  if v_currency not in ('IQD','USD') then
    raise exception 'fx_clearing_currency_invalid';
  end if;

  v_code:=case when v_currency='IQD' then 'FXI001' else 'FXU001' end;
  v_name:=case when v_currency='IQD'
    then 'FX transfer clearing IQD'
    else 'FX transfer clearing USD' end;

  -- Serialize first creation and reuse any historical account already owning
  -- the unique company/code pair. V7.5.2 only handled account-id conflicts,
  -- which caused PostgreSQL unique violations and an HTTP 409 response.
  perform pg_advisory_xact_lock(
    hashtextextended(p_company_id::text||':fx-clearing:'||v_currency,0)
  );
  ...
  v_id:='system-fx-clearing-'||lower(v_currency);
  ...
  values(
    p_company_id,v_id,v_code,v_name,'asset',v_currency,
    0,true,now(),now(),auth.uid()
  )
...
exception
  when unique_violation then
    -- A legacy account may have claimed the code concurrently. Resolve it
    -- deterministically instead of exposing a 409 Conflict to the browser.
    select account_id into v_id
      from public.erp_accounts
     where organization_id=p_company_id
       and lower(code)=lower(v_code)
...
create or replace function public.erp_transfer_cloud_cash_v4(
  p_company_id uuid,
...
revoke all on function public.erp_ensure_fx_clearing_account(uuid,text) from public,anon;
grant execute on function public.erp_ensure_fx_clearing_account(uuid,text) to authenticated,service_role;
revoke all on function public.erp_transfer_cloud_cash_v4(uuid,text,text,numeric,numeric,numeric,timestamptz,text) from public,anon;
grant execute on function public.erp_transfer_cloud_cash_v4(uuid,text,text,numeric,numeric,numeric,timestamptz,text) to authenticated,service_role;
